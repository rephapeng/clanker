...
func InferContext(question string) ServiceContext {
	...
	awsKeywords := []string{
		...
		"resource explorer", "resourcegroupstaggingapi", "tagged resources",
		"lambda layer", "lambda layers", "step functions", "state machine",
		"cloudformation", "cloud formation", "glue job", "glue jobs",
		"glue database", "data catalog", "emr cluster", "kinesis",
		"launch template", "wafv2", "ssm parameter", "ssm parameters",
		// AWS-specific terms
	...
	gcpKeywords := []string{
		...
		"cloud asset", "cloud asset inventory", "service usage", "enabled apis", "enabled services",
		"cloud run revision", "cloud run revisions", "cloud run worker pool", "cloud run worker pools",
		"cloud run domain mapping", "cloud run multi-region",
	}
